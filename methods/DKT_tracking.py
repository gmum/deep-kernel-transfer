...
class DKT(nn.Module):
    ...
    def get_model_likelihood_mll(self, train_x=None, train_y=None):
        ...
        if (train_x is None): train_x = torch.ones(32, 256).to(self.device)
        if (train_y is None): train_y = torch.ones(32).to(self.device)
    ...
    def optimize_step(self, epoch, iter, optimizer, data):
        ...
        if (torch.sum(mask) > 0) & (torch.sum(op_mask) == 50):
            optimizer.zero_grad()
            x1 = np.concatenate((np.expand_dims(np.arange(0.0, 3.2, 0.2), axis=1), np.ones((hist.shape[0], 1))), axis=1)
            x2 = np.concatenate((np.expand_dims(np.arange(0.0, 3.2, 0.2), axis=1), 1 + np.ones((hist.shape[0], 1))), axis=1)
            xf1 = np.concatenate((np.expand_dims(np.arange(3.2, 8.2, 0.2), axis=1), np.ones((fut.shape[0], 1))),
                                 axis=1)
            xf2 = np.concatenate((np.expand_dims(np.arange(3.2, 8.2, 0.2), axis=1), 1 + np.ones((fut.shape[0], 1))),
                                 axis=1)
            x = torch.tensor(np.concatenate((x1, x2), axis=0)).float().to(self.device)
            xf = torch.tensor(np.concatenate((xf1, xf2), axis=0)).float().to(self.device)
            x = torch.cat((x, xf), axis=0)
            hist = hist.to(self.device)
            nbrs = nbrs.to(self.device)
            mask = mask.bool().to(self.device)
            fut = fut.to(self.device)
            perm = torch.randperm(x.size(0))[:32]
            x = x[perm]
            z = self.feature_extractor(hist, nbrs, mask, x)
            labels = torch.cat((hist[:, 0, 0], hist[:, 0, 1]), axis=0)
            labelsf = torch.cat((fut[:, 0, 0], fut[:, 0, 1]), axis=0)
    ...
    def test_loop(self, data):  # no optimizer needed for GP
        ...
        if (torch.sum(mask) > 0) & (torch.sum(op_mask) == 50):
            ...
            MSES = []
            for k in TIMES:
                mse = torch.sqrt(torch.pow(pred.mean[k] - labelsf[k], 2) + torch.pow(pred.mean[25+k] - labelsf[25+k], 2))
