...
class DKT(nn.Module):
    def __init__(self, backbone, cnf, device, use_conditional, context_type,
                 num_tasks=1,  config=None, dataset='QMUL', n_support=5):
    ...
    def get_model_likelihood_mll(self, train_x=None, train_y=None):
        ...
        if self.num_tasks == 1:
            #likelihood = gpytorch.likelihoods.GaussianLikelihood(noise_constraint=gpytorch.constraints.Interval(0.4, 0.6))
    ...
    def train_loop(self, epoch, optimizer, params):
        ...
        for inputs, labels in zip(batch, batch_labels):
            ...
            if self.use_conditional:
                if self.context_type == 'nn_kernel':
                    context = self.model.kernel.model(z)
                else:
                    support_ids = list(np.random.choice(list(range(labels.shape[0])),
                                                        replace=False, size=self.n_support))
                    context = torch.cat([z[support_ids],
                                         labels[support_ids]])[:,0].unsqueeze(0).repeat(labels.shape[0], 1)
                y, delta_log_py = self.cnf(labels, context, torch.zeros(labels.size(0), 1).to(labels))
            ...
            loss = -self.mll(predictions, self.model.train_targets) + torch.sum(delta_log_py)
            loss.backward()
            optimizer.step()
            ...
            if self.use_conditional:
                ...
                new_means = sample_fn(predictions.mean.unsqueeze(1), context)
            else:
    ...
    def test_loop_qmul(self, n_support):  # no optimizer needed for GP
        ...
        with torch.no_grad():
            if self.use_conditional:
                if self.context_type == 'nn_kernel':
                    context_support = self.model.kernel.model(z_support)
                else:
                    context_support = torch.cat([z_support.squeeze(),
                                         labels.squeeze()])[:,0].unsqueeze(0).repeat(labels.shape[0], 1)
                y_support, _ = self.cnf(labels, context_support, torch.zeros(labels.size(0), 1).to(labels))
            else:
        ...
        with torch.no_grad():
            ...
            if self.use_conditional:
                if self.context_type == 'nn_kernel':
                    ...
                    context = torch.cat([z_support.squeeze(),
                                         labels.squeeze()]).unsqueeze(0).repeat(z_query.shape[0], 1)
                y, delta_log_py = self.cnf(y_all[n].unsqueeze(1), context,
            ...
            for k in range(pred.mean.shape[0]):
                ...
                if self.use_conditional:
                    flow_sample = sample_fn(sample.cuda(), self.model.kernel.model(z_query)[k].repeat(10000, 1))
                else:
                    flow_sample = sample_fn(sample.cuda())
                samples.append(sample)
                flow_samples.append(flow_sample)
                true_y.append(y_all[n][k])
                gauss_y.append(pred.mean[k])
                flow_y.append(new_means[k])
            path = '/home/mzieba/Projects/deep-kernel-transfer/save'
            plot_histograms(path, samples, true_y, gauss_y,  n, flow_samples, flow_y)
        ...
        with torch.no_grad():
            if self.use_conditional:
                if self.context_type == 'nn_kernel':
                    ...
                    context_support = torch.cat([z_support.squeeze(),
                                         labels.squeeze()]).unsqueeze(0).repeat(labels.shape[0], 1)
                y_support, _ = self.cnf(labels, context_support, torch.zeros(labels.size(0), 1).to(labels))
            else:
                y_support, _ = self.cnf(labels, torch.zeros(labels.size(0), 1).to(labels))
        y_support = y_support.squeeze()
